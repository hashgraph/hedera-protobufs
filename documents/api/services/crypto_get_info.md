...
| liveHashes | [LiveHash](#proto-LiveHash) | All of the livehashes attached to the account (each of which is a hash along with the keys that authorized it and can delete it) <blockquote>REVIEW NOTE<blockquote> Should this be deprecated? Live hashes do not appear to be available any longer.</blockquote></blockquote> |
| tokenRelationships | [TokenRelationship](#proto-TokenRelationship) | **Deprecated.** As of `HIP-367`, which enabled unlimited token associations, the potential scale for this value requires that users consult a mirror node for this information. |
