...
- [block_service.proto](#block_service-proto)
    ...
    - [PublishStreamRequest.EndOfStream](#com-hedera-hapi-block-PublishStreamRequest-EndOfStream)
...
| ----- | ---- | ----------- |
| block_items | [stream.BlockItem](#com-hedera-hapi-block-stream-BlockItem) | An ordered list of `BlockItem`s.<br/> This list supports sending block items to subscribers in batches for greater channel efficiency. |
...
| status | [PublishStreamRequest.EndOfStream](#com-hedera-hapi-block-PublishStreamRequest-EndOfStream) | An item sent when a stream is ended by the publisher. |
...
<a name="com-hedera-hapi-block-PublishStreamRequest-EndOfStream"></a>

### PublishStreamRequest.EndOfStream
A message sent to end a stream.

This request message SHALL be sent from a Publisher to a block
node system when a `publishBlockStream` stream ends.<br/>
This message SHALL be sent _at most_ once for each `publishBlockStream`
call.<br/>
The source system SHALL cease sending block items after sending
this item.


| Field | Type | Description |
| ----- | ---- | ----------- |
| status | [PublishStreamEndCode](#com-hedera-hapi-block-PublishStreamEndCode) | A status code. <p> This code indicates the reason the stream ended.<br/> This value MUST be set to a non-default value. |
| earliest_block_number | [uint64](#uint64) | The number of the earliest available block. <p> Publishers MUST provide the earliest available block still present in local cache. |
| latest_block_number | [uint64](#uint64) | The number of the last available block. <p> Publishers MUST provide the most recent available block. |
...
| Field | Type | Description |
| ----- | ---- | ----------- |
| items_hash | [bytes](#bytes) | A SHA2-384 hash of the `BlockItem`s received. <p> This field is REQUIRED.<br/> A source system MUST verify that this value matches its own internal calculated hash value, and MUST end the stream if the values do not match. |
...
| STREAM_END_RESET | 1 | The Publisher reached a reset point.<br/> No errors occurred and the source node orderly ended the stream.
